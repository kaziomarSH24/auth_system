    ...
    $(document).ready(function() {
      ...
      function viewConversation(conversationId) {
        ...
        $.ajax({
         ...
         <div class="direct-chat-msg ${message.sender_id == message.user_id ? 'right' : ''}">
                  <div class="direct-chat-infos clearfix">
                    <span class="direct-chat-name float-${message.sender_id == message.user_id ? 'right' : 'left'}">${message.sender_id == message.user_id ? message.sender : message.receiver}</span>
                    <span class="direct-chat-timestamp float-${message.sender_id == message.user_id ? 'left' : 'right'}">${message.created_at}</span>
                  </div>
      ...
      $('#sendBtn').on('click',function(e) {
        ...
        if (message.length > 0) {
          ...
          socket.emit('sendChatToServer', {id: conversationId, msg: message}); // send message to socket server
        ...
        socket.on(`sendChatToClient${conversationId}`, (message) => {
      $('.direct-chat-messages').append(`
        ...
        $.ajax({
          ...
          success: function(response) {
            ...
            if (response.status == 'success') {
              ...
              $('.direct-chat-messages').append(`
         <div class="direct-chat-msg ${response.message.sender_id == response.message.user_id ? 'right' : ''}">
                  <div class="direct-chat-infos clearfix">
                    <span class="direct-chat-name float-${response.message.sender_id == response.message.user_id ? 'right' : 'left'}">${response.message.sender_id == response.message.user_id ? response.message.sender : response.message.receiver}</span>
                    <span class="direct-chat-timestamp float-${response.message.sender_id == response.message.user_id ? 'left' : 'right'}">${response.message.created_at}</span>
                  </div>
                  <img class="direct-chat-img" src="{{asset('backend')}}/dist/img/user3-128x128.jpg" alt="message user image">
                  <div class="direct-chat-text">
